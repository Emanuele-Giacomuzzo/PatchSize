...
print(paste("!!", 
            ...
            "To make it easier to interpet, we showcase only one of the two disturbance levels.",
            "In this document we showcase only the", 
            disturbance_global_selected, 
            "disturbance)"))
```
...
##### Holistic {.tabset .tabset-pills}

```{r}
comparison_type = "all"
```

```{r child = here("3_r_files", "13_metaecos_stats_holistic.Rmd")}
```
...
```{r child = here("3_r_files", "18_ecos_stats_holistic.Rmd")}
```
