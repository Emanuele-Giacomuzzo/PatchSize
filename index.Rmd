...
```{r echo=FALSE, message=FALSE}
#Save environment
save.image(file = here("environments", "environment_before_small_patches_shannon.RData"))
```
...
```{r child = here("r_files", "large_patches_pop_density.Rmd")}
```

#### Pop Dominance {.tabset .tabset-pills}

```{r child = here("r_files", "large_patches_pop_dominance.Rmd")}
```

#### Pop Last Seen {.tabset .tabset-pills}

```{r child = here("r_files", "large_patches_pop_last_saw.Rmd")}
```
