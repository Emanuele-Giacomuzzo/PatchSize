...
```{r child = here("r_markdown_files", "12_create_ds_classes.Rmd"), eval = FALSE}
```

```{r child = here("r_markdown_files", "13_create_ds_classes_effect_size.Rmd"), eval = FALSE}
```
...
```{r child = here("r_markdown_files", "15_analyse_metaecos_variable.Rmd")}
```
...
```{r child = here("r_markdown_files", "16_analyse_ecos_variable.Rmd")}
```
...
```{r child = here("r_markdown_files", "17_analyse_ecos_BEF.Rmd")}
```

### Populations

```{r child = here("r_markdown_files", "18_analyse_ecos_populations.Rmd"), eval = FALSE}
```
...
```{r child = here("r_markdown_files", "19_save_plot_paper_metaecos.Rmd")}
```

```{r child = here("r_markdown_files", "20_save_plot_paper_S_L.Rmd")} 
```

```{r child = here("r_markdown_files", "21_save_plot_paper_M.Rmd")} 
```

```{r child = here("r_markdown_files", "22_save_plot_paper_unconnected_AH_ratio.Rmd")}
```

<details>
<summary>Figures to present</summary>
```{r child = here("r_markdown_files", "23_save_plot_present_metaecos.Rmd"), echo = FALSE}
```

```{r child = here("r_markdown_files", "24_save_plot_present_S_L.Rmd")} 
```
</details>
...
```{r child = here("r_markdown_files", "26_script_BEMOVI_video_analysis.Rmd"), class.source = "fold-show"}
```

### Species identification

```{r child = here("r_markdown_files", "27_script_BEMOVI_species_id.Rmd"), class.source = "fold-show"}
```
