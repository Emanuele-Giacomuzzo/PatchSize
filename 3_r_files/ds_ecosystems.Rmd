...
ds_for_evaporation = read.csv(here("2_data", "water_addition.csv")) %>%
  ...
  mutate(time_point = as.double(str_extract(time_point, "\\d+")))
...
ds_ecosystems <- ds_ecosystems %>%
  mutate(connection_type = case_when(str_detect(ecosystem_type, "connected to large") ~ "connected to large",
                                     str_detect(ecosystem_type, "connected to medium") ~ "connected to medium",
                                     str_detect(ecosystem_type, "connected to small") ~ "connected to small",
                                     str_detect(ecosystem_type, "unconnected") ~ "unconnected",
                                     TRUE ~ ""))
```
