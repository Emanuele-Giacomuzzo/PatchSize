...
"High disturbance"
plot.metaecos.points.3(data_for_analysis %>% filter(disturbance == "high"),
                       response_variable)

"Low disturbance"
plot.metaecos.points.3(data_for_analysis %>% filter(disturbance == "low"),
                       response_variable)
```

```{r}

# --- FIND MODEL OPTIMIZER --- #

model_type = "full"

optimizer_n_control_filtered = optizimers_n_methods %>%
  filter(disturbance == disturbance_global_input, 
         response_variable == response_variable,
         comparison == comparison_type,
         model_type == model_type)

optimizer_input = optimizer_n_control_filtered$optimizer
control_input = optimizer_n_control_filtered$control
```
...
full_model = lmer(get(response_variable) ~
                    ...
                    total_water_addition_ml * day +
                    baseline * day +
                    (day | system_nr),
                  data = data_for_analysis,
                  REML = FALSE,
                  control = lmerControl(optimizer = optimizer_input,
                                        optCtrl = list(algorithm = control_input)))
```
