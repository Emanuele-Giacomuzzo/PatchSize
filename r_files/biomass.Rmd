...
Okay great! Next step is to now integrate the predicted temporal dynamics.
...
Let's now integrate it into the mixed effect model!

```{r model-fitted}

ds_regional_yo = ds_regional_predicted %>%
  filter(metaecosystem_type == "S_L" | metaecosystem_type == "M_M", day > 0)

model = lm(regional_mean_bioarea ~ predicted_from_time + metaecosystem_type  + disturbance, data = ds_regional_yo)
summary(model)
```

Adjusted R-squared: 0.80.

```{r model-fitted-without-metaeco}

model = lm(regional_mean_bioarea ~ predicted_from_time + + disturbance, data = ds_regional_yo)
summary(model)
```

Adjusted R-squared: 0.74.

Okay, this was one way of doing it. Next way of doing this is to have two disturbance levels and construct two models and see there the effect size of meta-ecosystem type. And if that doesn't work either I should just look at different parts of the time series or look at a single time point.
