...
for (set_i in 1 : n_sets) {
  ...
  system_nrs_in_set_i = isolated_combinations_sets_filtered %>%
    filter(
      metaecosystem_type %in% metaecosystem_type_i,
      connection == "isolated",
      set == set_i
  ...
  filtered_data = ds_metaecosystems %>%
    filter(
      ...
      connection == "connected" |
        (connection == "isolated" & system_nr %in% system_nrs_in_set_i)
    )
  ...
  iterated_results_table$system_nr_isolated_systems[nrow(iterated_results_table)] = paste(system_nrs_in_set_i, collapse = " ")
}
```
