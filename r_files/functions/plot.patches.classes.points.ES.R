plot.patches.classes.points.ES = function(eco_metaeco_input,
                                       size_class_input){
    
    ds_classes_effect_size %>%
      filter(
        disturbance == disturbance_input,
        eco_metaeco_type %in% eco_metaeco_input,
      ...
      labs(
        ...
        y = paste(
          "Class",
          size_class_input,
          axis_names$axis_name[axis_names$variable == "abundance_hedges_d"]
        ),
        color = ""
      ) +
      scale_color_manual(
