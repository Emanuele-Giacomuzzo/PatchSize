...
ds_patches = ds_patches %>%
  mutate(bioarea_tot = bioarea_per_volume * (patch_size_volume * 1000)) %>% #multiply by 1000 because bioarea_per_volume is in µm2 and patch_size_vol is in ml 
  mutate(indiv_tot = indiv_per_volume * (patch_size_volume * 1000)) #multiply by 1000 because indiv_per_volume is in µm2 and patch_size_vol is in ml 
...
#Calculate alpha diversity (species richness, shannon, simpson, inv simpson)
