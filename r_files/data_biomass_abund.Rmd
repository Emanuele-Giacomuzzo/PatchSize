...
```{r import_population_data_(ds_biomass_abund), message = FALSE, echo = TRUE}
...
```{r tidy_time_points_(ds_biomass_abund), message = FALSE, echo = TRUE}
...
ds_biomass_abund = ds_biomass_abund %>%
  mutate(bioarea_tot = bioarea_per_volume * (patch_size_volume * 1000)) %>% #bioarea_per_volume in µm2, patch_size_vol in ml 
...
ds_biomass_abund = ds_biomass_abund %>%
  ...
  summarise(
    ...
    bioarea_tot = mean(bioarea_tot),
    indiv_per_volume = mean(indiv_per_volume),
    indiv_tot = mean(indiv_tot),
    Ble = mean(Ble),
    Cep = mean(Cep),
    Col = mean(Col),
    Eug = mean(Eug),
...
```{r calculate_alpha_diversity_(ds_biomass_abund)}

#Calculate species richness

ds_biomass_abund$shannon = NA
ds_biomass_abund$simpson = NA
ds_biomass_abund$inv_simpson = NA
for (row in 1:nrow(ds_biomass_abund)){
  
  species_vector = ds_biomass_abund %>%
    slice(row) %>%
    select(Ble:Tet)
  ...
  ds_biomass_abund[row,]$shannon = shannon
  ds_biomass_abund[row,]$simpson = simpson
  ds_biomass_abund[row,]$inv_simpson = inv_simpson
...
```{r add_baselines_(ds_biomass_abund)}

#Add baselines (from t1).

baselines = ds_biomass_abund %>%
  filter(time_point == 1) %>%
  group_by(culture_ID) %>%
...
ds_biomass_abund = full_join(ds_biomass_abund, baselines)
```
