...
```{r}
#Define the variables to which to group by 
vars_to_group_by = columns_patches[columns_patches != "culture_ID" & columns_patches != "system_nr"]
```

```{r}
#Calculate effect sizes
ds_patches_effect_size = calculate.effect.sizes(ds = ds_patches,
                                                vars_to_group_by = vars_to_group_by,
                                                vars_for_ES = variables_patches) %>%
  filter(time_point >= 1)
...
```{r}
#Add baselines. See explanation in ds_patches.
