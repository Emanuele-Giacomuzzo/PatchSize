...
```{r}
p_SL_SL_isolated_biomass = ds_metaecosystems %>%
  filter(disturbance == disturbance_input) %>%
...
```{r}
p_value_SL_SL_isolated_biomass = round(bootstrapped_statistics$p_values_mean, digits = 3)
delta_AIC_SL_SL_isolated_biomass = round(bootstrapped_statistics$delta_AIC_mean, digits = 2)
```