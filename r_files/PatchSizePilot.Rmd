...
number_for_pairing = rep(c(1:9),each=8)

S = cbind(S, number_for_pairing)

S = S%>%
  mutate(number_for_pairing = ...7)

L = cbind(L, number_for_pairing)
...
S_and_L = S_and_L %>%
  ...
  mutate(metaecosystem_type = "S_L_random") %>%
  mutate(system_nr = number_for_pairing)

ds_regional_to_merge = ds_regional %>%
  select(disturbance, day, time_point, metaecosystem_type, regional_mean_bioarea)

dataset_to_plot = rbind(S_and_L, ds_regional_to_merge)

dataset_to_plot %>%
  filter(metaecosystem_type == "S_L" | metaecosystem_type == "S_L_random") %>%
  ggplot(aes(x = day,
...
mixed_model = lmer(regional_mean_bioarea ~ 
                     metaecosystem_type * disturbance +
                     (1 | system_nr),
                   data = dataset_to_plot %>%
                     filter(metaecosystem_type == "S_L" | 
                              metaecosystem_type == "S_L_random") %>%
                     filter(time_point >= 2),
                   REML = FALSE)

null_model = lmer(regional_mean_bioarea ~
                    disturbance +
                    (1 | system_nr),
                   data = dataset_to_plot %>%
                    filter(metaecosystem_type == "S_L" | 
