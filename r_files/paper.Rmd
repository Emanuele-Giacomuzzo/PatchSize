...
To characterise protist communities, we employed the R-package BEMOVI (@Pennekamp2015) to extract the abundance and traits (e.g., speed, shape, size, see ref bemovi) of moving particles in the videos. The traits were then used to filter out particles that were not protists (e.g., particles that moved to fast to be a protist). Biomass was inferred from the sum of the area of the protists, named bioarea. Biodiversity was computed in two steps. First, we identified protist species using a support vector machine model (ref) that employed all extracted traits as predictor variables and protist monocultures as a training dataset. Second, we calculated biodiversity indices as follows. For each patch, we calculated evenness as Pielou index (ref) and biodiversity as Shannon Index (ref). For each meta-ecosystem, we calculated alpha diversity as the mean Shannon Index among patches, beta diversity as Bray-Curtis index among patches (ref), and gamma diversity as the number of species persisting in at least one of the two patches.

#### Statistical analysis

We measured how much the size of the connected patch mattered by comparing connected and isolated patches using an effect size. For each response variable, the treatment was the the connected patch (e.g., Shannon index of small patches connected to large) and the control was the isolated patch (e.g., Shannon index of small isolated patches). Effect sizes were measured as Hedge's d (ref), which is the difference between the mean of the treatment and the mean of the control, corrected for the variance of the pooled treatment and control (see Appendix). We then compared the effect size of connected patches to each other (e.g., we compared the effect size of small connected to large and small connected to small). 

When possible, response variables were analysed using mixed effect models (ref) using the r package lme4 (ref). Null models always included two effects: time as fixed effect and replicates as random effect. This random effect accounted for time having a different effect on the intercept and slope for different replicates. Intercept were set as correlated. However, response variables measured as effect sizes had replicates that were pooled, so the random effect of replicates couldn't be include. Therefore, we used linear models instead. Models were select according to the Akaike Information Criterion (AIC) (ref), computed using the AIC function of r package base (ref).

The effects of resource flow were captured by constructing systems composed of two isolated patches. We call these isolated systems. Each Small-Large isolated system was made of one small and one large isolated patches. Medium-Medium isolated systems were made of two medium isolated patches (which could not be the same patch). To compare a meta-ecosystem and an isolated system, we iterated multiple comparisons from which we derived a distribution of p-values and ΔAIC. Each iteration comprised isolated systems in which patches were combined differently (without resampling). The p-value and ΔAIC we present is the mean of their distribution.
