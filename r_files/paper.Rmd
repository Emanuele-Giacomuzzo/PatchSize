...
Title: Patch size drives the effect of resource flow on biodiversity and productivity
...
#### Size needs to be incorporated into meta-ecosystems

-   We have made great progress in studying productivity and function AND we are now incorporating also resource flows into it

-   BUT models have ecosystems of the same size, totally hovering over the great importance of patch size for ecology (donor, vector, receiving) , making theory too abstract, not applicable to real world ecosystem

    -   Patch size impacts the biotic and physical properties (examples)

    -   Patch size impacts many different organisation levels (examples), so their impact scale up to ecosystem level (examples)

-   THEREFORE we here incorporate patch size into meta-ecosystem theory so that we can then use it to understand the biodiversity and function of a real world landscape

#### Resource flows induce homogenization

#### Resource flows reduce productivity 

Size needs to be incorporated when looking at its effects on productivity

#### Evidences for the role of size in the production and incorporation of subsidies

-   Our study supports the Subsidized Island Biogeography Hypothesis (@Anderson2001).
...
```{r fig-connected-systems, fig.cap = figure_caption_3, fig.height = 9}
ggarrange(
