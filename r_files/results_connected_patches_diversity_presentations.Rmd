...
patch_type_all = c("Small connected to large",
                   "Small isolated",
...
plots[[1]] = plot.patches.empty.presentations(y_min_alpha,
                                              y_max_alpha,
                                              "shannon")

for (i in 1:length(patch_type_all)+1) {
  
  plots[[i+1]] = plot.patches.points.presentations(data = ds_patches,
                                                   patch_type_input = patch_type_all[1:i],
                                                   response_variable) +
...
for(i in 1:length(patch_type_all)+1){
  ...
  png(file = here("results", "figures", disturbance_global_input, "presentations", paste0("patches_connected_diversity_", i, ".png")),
