...
<center>[**Reduced model**]{.underline} [**(with `size_connected_patch`)**]{.underline}</center>

```{r}
reduced_model = lm(get(response_variable) ~
                  size_connected_patch,
                data = filtered_data)
```
...
```{r eval = eval_cook_distances}
# Create the cook's distance plot
cook_distances <- influence.measures(reduced_model)[["infmat"]] %>% 
  as.data.frame() %>%
  select(cook.d) %>%
  pull() %>%
  as.double()
...
#Create the residuals vs leverages plot
my_residuals <- resid(reduced_model)
my_leverages <- hatvalues(reduced_model)
filtered_data_to_plot$residuals = my_residuals
filtered_data_to_plot$leverages = my_leverages
leverage_plot = filtered_data_to_plot %>%
  plot_ly(x = ~leverages, 
...
#Calculate leverage points threshold (taken from rule of the statistical modelling course)
treshold_leverage_points = 2 * length(coef(reduced_model)) / nrow(filtered_data) 
treshold_leverage_points = round(treshold_leverage_points, digits = 2)

#Robust regression
#reduced_model_robust = lmrob(get(response_variable) ~ 
#                            size_connected_patch +
#                            size_connected_patch:day,
#                          data = filtered_data, 
#                          method = "MM")
#print(summary(reduced_model_robust), digits = 1)
#par(mfrow = c(2, 3))
#plot(reduced_model_robust, which = 1:5)
```
