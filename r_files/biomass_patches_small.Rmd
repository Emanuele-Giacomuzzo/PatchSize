...
```{r biomass-small-patches-boxplots}
for (disturbance_input in c("low", "high")) {
  print(
    ds_biomass_abund %>%
      group_by(culture_ID, patch_size, eco_metaeco_type, disturbance, day) %>%
      ...
      ggplot(
        aes(
          x = day,
          y = bioarea_per_volume_across_videos,
...
```{r biomass-small-patches-effect-size}
for (disturbance_input in c("low", "high")) {
  print(
    ds_patches_averaged %>%
      filter(!time_point == 0,
...
-   Full Model
    ...
    -   Adjusted R2 = `r R2_full`

-   Patch Type (includes: (1) Patch Type; (2) Patch Type \* Day; and (3) Patch Type \* Disturbance)
