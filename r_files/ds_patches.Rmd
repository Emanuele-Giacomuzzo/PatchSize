...
ds_patches = ds_patches %>%
  ...
  mutate(indiv_per_volume_Lynn = Ble + Cep + Col + Eug + Eup + Lox + Pau + Pca + Spi + Spi_te + Tet) %>%
  ungroup()
```
...
ds_patches = ds_patches %>%
  mutate(bioarea_per_ml = bioarea_per_volume / 1000,
         indiv_per_ml = indiv_per_volume / 1000,
         indiv_per_ml_Lynn = indiv_per_volume_Lynn / 1000,
         Ble = Ble / 1000,
         Cep = Cep / 1000,
         Col = Col / 1000,
         Eug = Eug / 1000,
...
```{r}
#Calculate how the community of a certain treatment diverges from the community of the control 
  ...
  for (time_point_input in first_time_point:last_time_point) {
    for(treatment_input in c("Small connected to large", "Large connected to small")){
      ...
      treatment_group = ds_patches %>%
        filter(eco_metaeco_type %in% treatment_input,
      ...
      control_group = ds_patches %>%
        filter(eco_metaeco_type %in% control_input,
               time_point == time_point_input,
               disturbance == disturbance_input) %>%
        select(culture_ID, 
               first_protist:last_protist)
