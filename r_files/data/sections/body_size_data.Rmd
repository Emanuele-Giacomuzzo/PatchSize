...
ds_classes_averaged = ds_classes %>%
  group_by(culture_ID, patch_size, eco_metaeco_type, disturbance, time_point, day, log_size_class) %>%
  summarise(
    log_abundance = mean(log_size_class_abundance),
    log_abundance_sd = sd(log_size_class_abundance),
    sample_size = n()
  ) %>%
  mutate(
  ...
  ) #Expected number of rows: 12 size classes * 109 cultures * 8 time points = 10464
...
```{r}
ds_classes_video_averaged = readRDS(here("results", "ds_classes_averaged.RData"))
