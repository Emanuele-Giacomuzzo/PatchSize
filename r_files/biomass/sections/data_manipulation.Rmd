...
ds_regional_biomass = ds_biomass %>%
  ...
  mutate(total_bioarea = bioarea_per_volume_video_averaged * patch_size_volume) %>%
  group_by(system_nr, 
           disturbance, 
           time_point,
           day, 
           ...
           patch_size_volume,
           metaecosystem_type) %>%
  summarise(total_regional_bioarea = sum(total_bioarea))
  
  #%>%
  #group_by(system_nr, disturbance, time_point, day, metaecosystem_type) %>%
  #summarise(regional_mean_bioarea = mean(bioarea_per_volume_video_averaged))

metaecosystems_to_take_off = 40 #System 40 was the system of culture 60 that I spilled
ds_regional_biomass = ds_regional_biomass %>%
...
```{r created-averaged-lnRR-ds, eval = FALSE}
