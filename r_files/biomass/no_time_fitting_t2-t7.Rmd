...
### Modify regional biomass data set
...
There is no random effect of meta-ecosystem number. Let's then keep the model without the random effects of system number (`no_system_nr_model`).

#### *Time random effects*

Should we keep the random effect of time?
...
Of course, time has an important effect. Let's then keep the model with time as random effect (`no_system_nr_model`).
...
There is no interaction between meta-ecosystem type and disturbance. Let's keep the model without such interaction (`no_interaction_model`)
...
Random slopes did not have an effect (but they had a low p-values still). Let's then keep the model without slopes (`no_slopes_model`). 
...
`lmer(regional_mean_bioarea ~ metaecosystem_type + disturbance  + (1 | day), data = ds_regional_shrunk_type_n_day, REML = FALSE)`
