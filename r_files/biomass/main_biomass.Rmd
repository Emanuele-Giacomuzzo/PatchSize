...
Also, I am going to select the best model acccording to BIC instead of AIC. This is because BIC is better for understanding and AIC for predicting. 

```{r child = "no_time_fitting_t2-t7.Rmd", echo = TRUE, eval = TRUE}
```

```{r child = "time_fitting.Rmd", echo = TRUE, eval = FALSE}
```

```{r child = "time_fitted_t2-t7.Rmd", echo = TRUE, eval = FALSE}
```

```{r child = "time_fitted_each_time_point.Rmd", echo = TRUE, eval = FALSE}
```
